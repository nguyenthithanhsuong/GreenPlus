...
export default function Home() {
  ...
  const runInsert = async () => {
    ...
    setMutationResult(null);
    try {
      const payload = parseJsonPayload(insertPayload);
      const { data, error: queryError } = await supabase
        .from(activeFeature.table)
        .insert([payload])
        .select();
      ...
      <div className="mx-auto max-w-7xl p-4 md:p-6">
        <div className="mb-6 rounded-xl border border-slate-800 bg-slate-900 p-5">
          ...
          <p className="mt-1 text-sm text-slate-300">
            Sidebar ben trai mo phong workflow cho Admin, Manager va Employee. Bam muc nao se hien du lieu select all cua bang Supabase tuong ung. Click bang de xem quan he.
          </p>
        </div>
          ...
          <main className="space-y-4 lg:col-span-3">
            ...
            <section className="rounded-xl border border-emerald-800/50 bg-gradient-to-br from-slate-900 to-slate-800/50 p-6">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-semibold text-slate-500 bg-slate-800 px-3 py-1 rounded-full">{activeFeature.group}</span>
              </div>
              <p className="text-sm text-slate-400 italic mb-3">{activeFeature.description}</p>
              {selectedRelatedId && (
                <div className="rounded-md bg-blue-900/30 border border-blue-700/50 px-3 py-2 text-xs text-blue-300">
