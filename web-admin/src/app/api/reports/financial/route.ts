...
export const GET = withSentry(async () => {
  ...
  const [batchesResult, paymentsResult] = await Promise.all([
    supabaseServer
      .from("batches")
      .select(
        "batch_id,quantity,import_price,created_at,products(name),suppliers(name)",
      )
      .order("created_at", { ascending: false }),
    supabaseServer
      .from("payments")
      .select(
        "payment_id,order_id,method,status,amount,payment_date,orders(order_id,order_date,users(name))",
      )
      .order("payment_date", {
        ascending: false,
        nullsFirst: false,
  ...
  const batches = ((batchesResult.data ?? []) as BatchReportRow[]).map(
    (batch) => ({
      ...batch,
  ...
  const payments = ((paymentsResult.data ?? []) as PaymentReportRow[]).map(
    (payment) => ({
      ...payment,
  ...
  for (const batch of batches) {
    ...
    transactions.push({
      ...
      kind: "batch_import",
      title: batch.products?.name
        ? `Nhập lô ${batch.products.name}`
        : "Nhập lô hàng",
  ...
  logger.info("Get financial report success", {
