...
export async function POST(request: Request) {
  try {
    ...
    if (!ALLOWED_MIME.has(file.type)) {
      logger.error("Product image upload failed - invalid mime type", { mimeType: file.type });
    ...
    if (uploadError) {
      logger.error("Product image upload failed - storage error", { error: uploadError.message });
    ...
    logger.error("Product image upload unexpected error", { error: toErrorMessage(error) });
    return NextResponse.json(
